 ...
 */
export function createAuthProvider<T extends AuthClient>(AuthContext: React.Context<IAuthContextProps<T>>) {
  ...
  return class KeycloakProvider extends React.PureComponent<AuthProviderProps<T>, AuthProviderState> {
    state = {
      ...initialState,
    };
    ...
    componentDidUpdate({ authClient: prevAuthClient, initOptions: prevInitOptions }: AuthProviderProps<T>) {
    ...
    updateState = (event: AuthClientEvent) => () => {
      ...
      const { initialized: prevInitialized, isAuthenticated: prevAuthenticated, isLoading: prevLoading } = this.state;
    ...
    render() {
      ...
      return <AuthContext.Provider value={{ initialized, authClient }}>{children}</AuthContext.Provider>;
    }
  };
}
